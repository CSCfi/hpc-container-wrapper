...
from cw_common import print_err  # noqa: E402
...
def add_post_flag(par):
    par.add_argument(
        "--post-install",
        help="Script to run after initial setup",
...
def add_pre_flag(par):
    par.add_argument(
        "--pre-install",
        help="Script to run before initial setup",
        type=lambda x: is_valid_file(x),
    )
...
def add_env_flag(par):
    par.add_argument(
        "--environ",
        help="Script to run before each program launch ",
        type=lambda x: is_valid_file(x),
    )
...
def get_old_conf(d, conf):
    ...
    try:
        ...
        print_err(
            "Directory {} does not exist or is not a valid installation ( missing share/conf.yaml )".format(
                d
            )
        )
        sys.exit(1)
